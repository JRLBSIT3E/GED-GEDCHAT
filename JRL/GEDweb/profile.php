...
<body>
    <div class="container">
        ...
        if($flag == 0) { // Your Own Profile       
            $postsql = "SELECT posts.post_caption, posts.post_time, users1.user_firstname, users1.user_lastname,
                                posts.post_public, users1.user_id, users1.user_gender, users1.user_nickname,
                                users1.user_birthdate, users1.user_hometown, users1.user_status, users1.user_about, 
                                posts.post_id
                        FROM posts
                        JOIN users1
                        ON users1.user_id = posts.post_by
                        WHERE posts.post_by = $current_id
                        ORDER BY posts.post_time DESC";
            $profilesql = "SELECT users1.user_id, users1.user_gender, users1.user_hometown, users1.user_status, users1.user_birthdate,
                                 users1.user_firstname, users1.user_lastname
                          FROM users1
                          WHERE users1.user_id = $current_id";
            $profilequery = mysqli_query($conn, $profilesql);
        } else { // Another Profile ---> Retrieve User data and friendship status
            $profilesql = "SELECT users1.user_id, users1.user_gender, users1.user_hometown, users1.user_status, users1.user_birthdate,
                                    users1.user_firstname, users1.user_lastname, userfriends.friendship_status
                            FROM users1
                            LEFT JOIN (
                            ...
                            ON userfriends.user_id = users1.user_id
                            WHERE users1.user_id = $current_id";
            $profilequery = mysqli_query($conn, $profilesql);
