...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // A form is posted
    if (isset($_POST['login'])) { // Login process
        ...
        $query = mysqli_query($conn, "SELECT * FROM users1 WHERE user_email = '$useremail' AND user_password = '$userpass'");
        if($query){
    ...
    if (isset($_POST['register'])) { // Register process
        ...
        $query = mysqli_query($conn, "SELECT user_nickname, user_email FROM users1 WHERE user_nickname = '$usernickname' OR user_email = '$useremail'");
        ...
        // Insert Data
        $sql = "INSERT INTO users1(user_firstname, user_lastname, user_nickname, user_password, user_email, user_gender, user_birthdate, user_status, user_about, user_hometown)
        ...
        if($query){
            $query = mysqli_query($conn, "SELECT user_id FROM users1 WHERE user_email = '$useremail'");
            $row = mysqli_fetch_assoc($query);
            $_SESSION['user_id'] = $row['user_id'];
            header("location:home.php");
        }
