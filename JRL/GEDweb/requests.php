...
<body>
    <div class="container">
        ...
        ?>
        <?php
        $sql = "SELECT users1.user_gender, users1.user_id, users1.user_firstname, users1.user_lastname
                FROM users1
                JOIN friendship
                ON friendship.user2_id = {$_SESSION['user_id']} AND friendship.friendship_status = 0 AND friendship.user1_id = users1.user_id";
        $query = mysqli_query($conn, $sql);
        $width = '168px';
        $height = '168px';
        if(!$query)
